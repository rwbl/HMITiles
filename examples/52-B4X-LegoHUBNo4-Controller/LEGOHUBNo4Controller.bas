...
@EndOfDesignText@
#Region Class Header
' Class:		LegoHUBNo4Controller.bas
...
Sub Class_Globals
	...
	' BLE Device
	' ============================================================
	Public BLE_MAC As String 			= "90:84:2B:C2:67:E8"						' LegoHUBNo4 MAC address
...
Public Sub SetOutputLevel
	DoLog($"[LEGOHUBNo4Controller.SetOutputLevel] level=${OUTPUT_LEVEL_DEFAULT}"$)
	mMainpage.Write(Array As Byte(COMMAND_OUTPUT_LEVEL, OUTPUT_LEVEL_DEFAULT))
	Sleep(COMMAND_DELAY)
End Sub
...
Public Sub SetPowerLevel(port As Byte, level As Int)
	DoLog($"[LEGOHUBNo4Controller.SetPowerLevel] port=${port}, level=${level}"$)
...
Public Sub GetDistance(data() As Byte) As Int
	...
	If raw > 1023 Then
		' DoLog("[GetDistance] Out of Range raw=", raw)
...
Public Sub MessageLogger(data() As Byte)
	...
	If EnableDebugHex Then
		DoLog("[Client_NewData] len=" & data.Length)
		DoLog("[Client_NewData] raw=" & bc.HexFromBytes(data))
	End If
	...
	Select msgType
		...
		Case MESSAGE_TYPE_PORT_VALUE_SINGLE, _
			...
			DoLog("[LEGOHUBNo4Controller.Sensor] raw=" & BytesToHex(data) & ", length=" & data.length)

			' --- Distance detection (Mode 0x01) ---
			If IsDistance(data) Then
				Dim dist As Int = GetDistance(data)
				If dist == DISTANCE_OUT_OF_RANGE Then
					DoLog("[LEGOHUBNo4Controller.Distance] Out of Range")
				Else
					DoLog("[LEGOHUBNo4Controller.Distance] dist=" & dist & " units")
				End If
			End If
			...
			If IsColor(data) Then
				...
				DoLog("[LEGOHUBNo4Controller.Color] color=" & color & " / " & GetColorName(color))
			End If

			' ===== Port Command Feedback (0x82) =====
		Case MESSAGE_TYPE_COMMAND_FEEDBACK
			If ParseRawDataCommandFeedback(data) Then
				DoLog("[LEGOHUBNo4Controller.CommandFeedback] hubid=" & OneByteToHex(MsgCommandFeedback.HubId) & _
		...
		Case MESSAGE_TYPE_HUB_ATTACHED_IO
			If ParseRawDataHubAttachedIO(data) Then
				DoLog("[LEGOHUBNo4Controller.HubAttachedIO] event=" & OneByteToHex(MsgHubAttachedIO.Event) & _
		...
		Case Else
			DoLog("[LEGOHUBNo4Controller.Unknown] msgType=" & OneByteToHex(msgType) & _
...
#Region Helper
Private Sub DoLog(text As String)
	If Logging Then
		Log(text)
	End If
End Sub
