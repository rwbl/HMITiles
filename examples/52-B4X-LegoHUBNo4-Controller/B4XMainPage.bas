...
' Brief:		B4X, B4J Client controlling a Lego HUB No4 Brick via BLE using HMI tiles.
' Date:			2026-01-05
...
'				- Samsung Galaxy SM-A136B (A13 5G) is 720 x 1600 pixels (HD+). 
'				- Samsung Galaxy A13, 6.5-inch display, 1080 x 2408 px, Android 14, One UI 6.1
'				B4J:
...
Private Sub Class_Globals
	Private Const VERSION As String	= "Lego HUB No4 Controller v20260105"
...
Private Sub B4XPage_Created (Root1 As B4XView)
	...
	Hub.Initialize(B4XPages.GetPage("Mainpage"))
...
Private Sub BLEScanTimer_Tick
	...
	If BLEScanTimerCounter > Round(BLE_SCAN_TIMER_TIMEOUT / BLE_SCAN_TIMER_INTERVAL) And Not(IsDeviceFound) Then
		BLEMgr.StopScan
		BLEScanTimer.Enabled = False
		TileEventViewer.Insert($"[BLEScanTimer_Tick] Device ${Hub.BLE_DEVICE_NAME } not found. Timeout reached."$, HMITileUtils.EVENT_LEVEL_ALARM)
		TileButtonConnectUpdate(IsConnected)
	End If
End Sub
...
Public Sub Connect
	...
	Else
		...
		BLEMgr.Scan2(Array As String(Hub.SERVICE_UUID), False)
	End If
End Sub
...
'	Log($"[BLEMgr_Connected] services=${services}"$)
	...
	BLEMgr.SetNotify(Hub.SERVICE_UUID.ToLowerCase, Hub.CHAR_UUID_RX.ToLowerCase, True)
	Sleep(10)
...
Public Sub Write(data() As Byte)
	BLEMgr.WriteData(Hub.SERVICE_UUID, Hub.CHAR_UUID_TX, data)
End Sub
#End If
#End Region
