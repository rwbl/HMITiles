...
Public Sub IntToByte(value As Int) As Byte
	If value >= 0 And value <= 255 Then
		Return value
	End If
	' Log($"[Convert.IntToByte] int out of range (0–255): ${value}. Using 0."$)
	Return 0
End Sub
...
Public Sub PrintMap(m As Map)
	For Each key As String In m.Keys
		Log($"[Convert.PrintMap] key=${key}, value=${m.Get(key)}"$)
	Next
End Sub
...
Public Sub MapRange(Value As Float, FromLow As Float, FromHigh As Float, ToLow As Float, ToHigh As Float) As Float
	Return ToLow + (ToHigh - ToLow) * ((Value - FromLow) / (FromHigh - FromLow))
End Sub

'Map the value from the 'From' range A and B to the 'To' range C and D.
Public Sub MapRangeInt(Value As Int, FromLow As Int, FromHigh As Int, ToLow As Int, ToHigh As Int) As Int
	Return ToLow + (ToHigh - ToLow) * ((Value - FromLow) / (FromHigh - FromLow))
End Sub
