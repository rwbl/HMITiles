...
Sub Class_Globals
	...
	Private TileButtonServoState 	As HMITileButton
	Private TileReadoutServoState 	As HMITileReadout
...
Private Sub SetTilesInitialState
	TileButtonConnect.ValueFontFontAwesome
	TileButtonConnect.OnText = Chr(0xF0C1)
	TileButtonConnect.OffText = Chr(0xF127)
	TileButtonConnect.Value = IsConnected
	TileButtonConnectUpdate(IsConnected)
	
	' Servo state is open
	TileButtonServoState.Value = True
	TileReadoutServoState.Value = IIf(TileButtonServoState.Value, "Open", "Closed")
End Sub
...
Private Sub TileButtonConnectUpdate(state As Boolean)
	...
	TileButtonConnect.Value = state
End Sub

Private Sub TileButtonServoState_Click
	TileButtonServoState.Value = Not(TileButtonServoState.Value)
	If TileButtonServoState.Value Then
		BleWrite(CMD_OPEN)
